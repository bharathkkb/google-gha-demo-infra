...
output "get_credentials_command" {
  ...
  value       = format("gcloud container clusters get-credentials --project %s --zone %s --internal-ip %s", module.gke-project.project_id, module.gke.location, module.gke.name)
}
...
output "bastion_ssh_command" {
  ...
  value       = format("gcloud beta compute ssh %s --tunnel-through-iap --project %s --zone %s -- -L8888:127.0.0.1:8888", module.bastion.hostname, module.gke-project.project_id, local.bastion_zone)
}
